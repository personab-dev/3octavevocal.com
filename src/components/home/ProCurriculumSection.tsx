      ...
      <div className="max-w-5xl mx-auto px-6">
        ...
        >
          ...
          <h2 className="text-2xl md:text-3xl lg:text-4xl font-bold text-text-on-light">
            <span className="text-accent">같은 방식 그대로 성장</span>할 수 있습니다.
          </h2>
        </motion.div>
        ...
        >
          ...
          <p>결국, <strong className="text-text-on-light">실력이 올라가는 방법</strong>을 선택할 수 밖에 없습니다.</p>
        </motion.div>
        ...
        <div className="grid grid-cols-2 md:grid-cols-4 gap-4 mb-10">
          {Array.from({ length: 4 }).map((_, i) => (
            <motion.div
              ...
              className="aspect-[3/4] bg-zinc-200 relative overflow-hidden group"
            >
              <div className="absolute inset-0 bg-gradient-to-t from-black/50 to-transparent" />
              <span className="absolute inset-0 flex items-center justify-center text-zinc-400 text-xs tracking-widest uppercase">
                Artist {i + 1}
              </span>
            </motion.div>
          ))}
        ...
        >
          <div>
            <p>결과가 검증된 시스템, 이미 수많은 연예인과 아티스트들이 실력 향상을 위해 이곳을 찾고 있습니다.
            &apos;더 글로리&apos; 주연 배우부터, 신화·비스트·인피니트·노라조까지!</p>
            <p className="text-accent font-bold mt-2">프로들이 직접 검증한 커리큘럼 그대로!</p>
          </div>
          <div>
